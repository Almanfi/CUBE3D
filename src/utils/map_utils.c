...
/*   Updated: 2023/07/04 18:59:02 by bamrouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*copy_line_and_fill(char *line, size_t len)
{
	...
	if (i < len)
	{
		new_line = ft_malloc((len + 1)* sizeof(char), (t_mem_param){0,GNL_SCOPE, NULL, 0});
		if (!new_line)
...
static void    fill_lines(t_cub3d *cub3d,size_t max_len)
{
    ...
    {
		cub3d->map_content[i] = copy_line_and_fill(cub3d->map_content[i], max_len);
        i++;
    }
    temp_content = cub3d->map_content;
	cub3d->map_content = add_element_to_array(cub3d->map_content, &cub3d->map_content[0], sizeof(char *));
